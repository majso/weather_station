...
bool sht30_soft_reset() {
    uint8_t cmd[2] = {SHT30_SOFT_RESET >> 8, SHT30_SOFT_RESET & 0xFF}; // Using defined constant
...
bool sht30_read_data(float *temperature, float *humidity) {
    uint8_t cmd[2] = {SHT30_MEASURE_HIGHREP_STRETCH >> 8, SHT30_MEASURE_HIGHREP_STRETCH & 0xFF}; // Using defined constant
    uint8_t data[6];
